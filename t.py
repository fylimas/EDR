...
#更改系统参数
w=0.5
A_min = np.array([[0,1],[-w*w,0]])
w=1.5
A_max = np.array([[0,1],[-w*w,0]])
...
K_n = np.array([[a*a,0]])
#新控制器
参环t, 参环y, 参环x_n = ct.forced_response(ct.StateSpace(A_min - A + K_n, B, C, D),T=np.linspace(0, 时长, 1000),U=(N+a*a)*r,return_x=True)
参变t, 参变y, 参变x_min_n = ct.forced_response(ct.StateSpace(A_min - A + K_n, B, C, D),T=np.linspace(0, 时长, 1000),U=(N+a*a)*r,return_x=True)
参变t, 参变y, 参变x_max_n = ct.forced_response(ct.StateSpace(A_min - A + K_n, B, C, D),T=np.linspace(0, 时长, 1000),U=(N+a*a)*r,return_x=True)
